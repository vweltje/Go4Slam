...
class Content extends MY_Controller {
	...
	public function index() {
		...
		$data['blog_posts'] = $this->blog_posts_model->get_all();
	...
	public function delete_blogpost($post_id = false) {
		$data['success'] = false;
		if ($post_id) {
			if ($this->timeline_model->delete(array('type' => 'blog_post', 'item_id' => $post_id))) {
				if ($this->blog_posts_model->delete($post_id)) {
					$data['success'] = true;
				}
			}
		}
		$this->session->set_flashdata('message', 'Blog post successfully deleted.');
		redirect($this->agent->referrer());
	}
}
